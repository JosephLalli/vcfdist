...
4. **Precision / recall** — `precision_recall_threads_wrapper` in `dist.cpp` drives all precision/recall work through a single `PrThreadPool` of `max_threads - 1` worker threads (the calling thread participates as the 64th worker). Superclusters across all RAM tiers compete for work via a shared atomic counter. For each supercluster, the four independent query/truth haplotype alignments are submitted as pool tasks; the dispatching thread spin-steals (`try_steal_exec`) from the same queue rather than blocking, preventing deadlock while keeping all threads busy. This replaces the previous three-level architecture (per-tier `nthreads` outer threads, conditionally nested 4-alignment threads, a global atomic reservation budget) and eliminates thread creation churn and oversubscription.

   Each per-alignment call to `calc_prec_recall_aln` selects between two backends based on alignment cell count and the `--exact-prec-recall` flag:
   - **Default (WFA corridor)**: when `cell_count > PR_WFA_CORRIDOR_THRESHOLD` (1,000,000) and `--exact-prec-recall` is not set, `calc_prec_recall_aln_one_wfa` runs a banded WFA sweep to discover active diagonals, then restricts the dense state-matrix fill to those corridor diagonals. This avoids allocating the full O(query_len * truth_len) matrix for large superclusters. On the full-chr2 fixture the PR stage completes in ~14.6 s at 64 threads vs. ~43.8 s exact. The tradeoff: the corridor builds a leaner co-optimal pointer DAG, so the backward max-TP pass may undercount true-positive credit by a small deterministic amount on tie-rich superclusters (observed: 3 INDEL TPs on the full-chr2 fixture).
   - **Exact (`--exact-prec-recall` / `-ep`)**: forces every alignment through the full dense state-matrix BFS regardless of size, maximizing true-positive credit. Output is byte-identical to the pre-corridor implementation. Use this when benchmark comparisons require the exact golden metrics.
